 ...
 * Accessed at /tablet
 *
 * AUTH: PIN verification is done server-side via /api/auth/tablet
 * which checks against bcrypt-hashed pin_hash in the database.
...
function TabletApp() {
  ...
  const [yardStaff, setYardStaff] = useState<Database["public"]["Tables"]["yard_staff"]["Row"][]>([])
  ...
  // Active session (after successful auth)
  ...
  async function handlePunch() {
    ...
    // Step 1: Verify PIN via server API (on clock IN, or if no active session)
    if (action === 'IN') {
      try {
  ...
  // Handle name selection from big buttons
  function handleSelectName(name: string, id: string) {
        ...
        <div className="lg:col-span-1">
          <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 shadow-xl">
            ...
            <div className="flex gap-2 mb-6">
              <button
                onClick={() => { setMode('drivers'); setSelectedName(''); setSelectedPersonId('') }}
              ...
              <button
                onClick={() => { setMode('yard'); setSelectedName(''); setSelectedPersonId('') }}
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {people.map(p => {
                ...
                return (
                  <button
                    key={p.id}
                    onClick={() => handleSelectName(name, p.id)}
