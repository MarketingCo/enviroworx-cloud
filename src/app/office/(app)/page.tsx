...
export default function OfficePage() {
  ...
  async function handleLogout() {
    ...
    sessionStorage.removeItem('office_session')
    router.replace('/office/login')
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex gap-2">
              <a
                href="/driver"
                target="_blank"
                className="text-[10px] font-bold uppercase tracking-widest text-slate-500 hover:text-white px-2 py-1 rounded bg-slate-900 border border-white/5"
              >
                Driver
              </a>
              <a
      ...
      {isConfigMissing && (
        <div className="bg-yellow-500/10 border-b border-yellow-500/20 px-6 py-3 text-center">
          <p className="text-yellow-400 text-xs font-bold">
            Supabase not configured — add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to
            .env.local
          </p>
        </div>
      )}
