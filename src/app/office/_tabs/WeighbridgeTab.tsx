...
export default function WeighbridgeTab() {
  const [tippers, setTippers] = useState<Database["public"]["Tables"]["active_tippers"]["Row"][]>([])
  ...
  const [suggestions, setSuggestions] = useState<Database["public"]["Tables"]["customers"]["Row"][]>([])
  ...
  useEffect(() => {
    ...
    const wbCh = supabase.channel('wb-readings').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'weighbridge_readings' }, (p) => {
      setLiveWeight((p.new as { weight_kg: number }).weight_kg)
    }).subscribe()
  ...
  function selectSuggestion(c: Database["public"]["Tables"]["customers"]["Row"]) {
  ...
  function loadFromTipper(t: Database["public"]["Tables"]["active_tippers"]["Row"]) {
    setForm(f => ({
      ...f,
      lorryReg: t.reg,
    ...
    if (result.success) {
      toast.success((t) => (
        <div className="flex flex-col gap-2">
          ...
          <button
            onClick={() => {
              window.open(`/api/documents?type=WTN&ticketNumber=${(result as { ticketNumber?: string }).ticketNumber}`, '_blank')
              toast.dismiss(t.id)
            }}
      ...
      <div className="lg:col-span-2 space-y-4">
        <div className="bg-slate-900 border border-white/5 rounded-xl p-5">
          ...
          <div className="space-y-2 mt-3">
            {tippers.map((t: Database["public"]["Tables"]["active_tippers"]["Row"]) => {
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {lorries.map((l: Database["public"]["Tables"]["lorries"]["Row"]) => <option key={l.id} value={l.registration}>{l.registration}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            {suggestions.length > 0 && (
              <div className="absolute z-50 w-full bg-slate-800 border border-white/20 rounded-lg mt-1 shadow-[0_20px_50px_rgba(0,0,0,0.5)] max-h-64 overflow-y-auto">
                {suggestions.map((c: Database["public"]["Tables"]["customers"]["Row"]) => (
