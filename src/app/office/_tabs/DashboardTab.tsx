...
export interface DashStats {
  ...
  inventorySummary: Database["public"]["Views"]["v_inventory_summary"]["Row"][]
  activeTippers: Database["public"]["Tables"]["active_tippers"]["Row"][]
  unpaidInvoices: Database["public"]["Views"]["v_unpaid_invoices"]["Row"][]
  driverHours: Database["public"]["Views"]["v_driver_hours_today"]["Row"][]
  collections: Database["public"]["Views"]["v_collections_due"]["Row"][]
  expiringPermits: Database["public"]["Tables"]["permits"]["Row"][]
}

// ─── Dashboard Tab ───────────────────────────────────────────────────────────

export default function DashboardTab({ data, onRefresh }: { data: DashStats | null; onRefresh: () => void }) {
  if (!data) return <div className="flex items-center justify-center h-64 text-slate-500 text-sm">Loading dashboard...</div>

  const totalUnpaid = data.unpaidInvoices.reduce((s: number, i: Database["public"]["Views"]["v_unpaid_invoices"]["Row"]) => s + (i.amount || 0), 0)
        ...
        <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 flex items-start gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {expiringPermits.map((p: Database["public"]["Tables"]["permits"]["Row"]) => (
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 border border-white/5 rounded-xl p-5">
          ...
          <div className="space-y-2">
            ...
            {data.inventorySummary.map((row: Database["public"]["Views"]["v_inventory_summary"]["Row"], i: number) => (
        ...
        <div className="bg-slate-900 border border-white/5 rounded-xl p-5">
          ...
          {data.activeTippers.length === 0
            ...
            : <div className="space-y-2">
                {data.activeTippers.map((t: Database["public"]["Tables"]["active_tippers"]["Row"], i: number) => (
        ...
        <div className="bg-slate-900 border border-white/5 rounded-xl p-5">
          ...
          {data.driverHours.length === 0
            ...
            : <div className="space-y-2">
                {data.driverHours.map((d: Database["public"]["Views"]["v_driver_hours_today"]["Row"], i: number) => (
                  <div key={i} className="flex justify-between text-sm">
                    <span className="text-white font-bold">{d.employee}</span>
                    <span className={`font-black ${(d.hours_worked ?? 0) >= 8.5 ? 'text-yellow-400' : 'text-primary'}`}>
        ...
        <div className="bg-slate-900 border border-white/5 rounded-xl p-5">
          ...
          <div className="space-y-2 max-h-52 overflow-y-auto">
            {data.unpaidInvoices.length === 0
              ...
              : data.unpaidInvoices.map((inv: Database["public"]["Views"]["v_unpaid_invoices"]["Row"], i: number) => (
        ...
        <div className="bg-slate-900 border border-white/5 rounded-xl p-5">
          ...
          <div className="space-y-2 max-h-52 overflow-y-auto">
            {data.collections.length === 0
              ...
              : data.collections.map((c: Database["public"]["Views"]["v_collections_due"]["Row"], i: number) => (
                  <div key={i} className="flex justify-between items-center text-sm border-b border-white/5 pb-2">
                    <div>
