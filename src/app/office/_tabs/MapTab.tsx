...
export default function MapTab() {
  const [skips, setSkips] = useState<Database["public"]["Tables"]["inventory"]["Row"][]>([])
  const [vehicles, setVehicles] = useState<Database["public"]["Tables"]["vehicles"]["Row"][]>([])
  const [liveOrders, setLiveOrders] = useState<Database["public"]["Tables"]["orders"]["Row"][]>([])
  const [externalPoints, setExternalPoints] = useState<Database["public"]["Tables"]["external_map_points"]["Row"][]>([])
  ...
  function initMap() {
    ...
    const L = (window as unknown as { L?: { DomUtil: { get: (id: string) => HTMLElement | null }; map: (id: string) => unknown; tileLayer: (url: string, opts: Record<string, string>) => unknown; divIcon: (opts: Record<string, unknown>) => unknown; marker: (coords: [number, number], opts?: Record<string, unknown>) => { addTo: (m: unknown) => unknown; bindPopup: (html: string) => unknown }; featureGroup: (markers: unknown[]) => { getBounds: () => { pad: (n: number) => unknown } }; control: { layers: (baseLayers: Record<string, unknown>, overlays: Record<string, unknown>) => { addTo: (m: unknown) => unknown } } } }).L
    if (!L) return
    const container = L.DomUtil.get('office-map-canvas')
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if (container) (container as unknown as { _leaflet_id: null | number })._leaflet_id = null
