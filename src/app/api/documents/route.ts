...
interface LineItem extends OrderRow {
  net: number
  vat: number
  gross: number
}
...
async function generateWTN(params: URLSearchParams) {
  ...
  const wl = wlRaw as WeightLogRow
...
async function generateInvoice(params: URLSearchParams) {
  ...
  if (orderId) {
    ...
    const { data: ords } = await supabaseAdmin.from('orders')
      .select('*')
      .eq('customer_id', cust.id)          // Phase 9: use FK join (replaces ilike on customer_name)
      .eq('status', 'Completed')
      .eq('paid', false)
  ...
  const lineItems = orders.map((o: OrderRow) => {
  ...
  const totalNet = lineItems.reduce((s: number, l: LineItem) => s + l.net, 0)
  const totalVat = lineItems.reduce((s: number, l: LineItem) => s + l.vat, 0)
  const totalGross = lineItems.reduce((s: number, l: LineItem) => s + l.gross, 0)
