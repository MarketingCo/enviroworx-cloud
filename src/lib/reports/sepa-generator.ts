...
export function generateSepaXml(invoices: any[], messageId: string) {
...
<Document xmlns="urn:iso:std:iso:20022:tech:xsd:pain.008.001.02">
  <CstmrDrctDbtInitn>
    <GrpHdr>
      ...
      <InitgPty>
        <Nm>${(DEFAULT_CONFIG as any).companyName || 'Enviroworx'}</Nm>
      </InitgPty>
    </GrpHdr>
    <PmtInf>
      ...
      <CdtrAcct>
        <Id>
          <Othr>
            <Id>${(DEFAULT_CONFIG as any).accountNumber || '00000000'}</Id>
          </Othr>
        </Id>
      </CdtrAcct>
      <CdtrAgt>
        <FinInstnId>
          <Othr>
            <Id>${(DEFAULT_CONFIG as any).sortCode?.replace(/-/g, '') || '000000'}</Id>
          </Othr>
        </FinInstnId>
      </CdtrAgt>
      <CdtrSchmeId>
